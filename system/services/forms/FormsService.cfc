		...
		for( var tab in frm.tabs ){
			...
			for( var fieldset in tab.fieldsets ) {
				...
				for( var field in fieldset.fields ) {
					...
					if ( !readOnly && !deleted && !ArrayContainsNoCase( ignoreControls, control ) && !ArrayContainsNoCase( arguments.suppressFields, field.name ) ) {
		...
		for( var tab in frm.tabs ){
			...
			for( var fieldset in tab.fieldsets ) {
				...
				for( var field in fieldset.fields ) {
					...
					if ( !IsBoolean( field.autoTrim ?: "" ) || ArrayContainsNoCase(ignoreControls, control ) || deleted || ArrayContainsNoCase( arguments.suppressFields, field.name ) ) {
						continue;
					}
					if ( field.autoTrim ) {
	...
	public string function renderForm(
		...
		for( var tab in frm.tabs ){
			...
			for( var fieldset in tab.fieldsets ) {
				...
				for( var field in fieldset.fields ) {
					if ( ( IsBoolean( field.deleted ?: "" ) && field.deleted ) || ArrayContainsNoCase( arguments.suppressFields, field.name ) ) {
						continue;
					}
		...
		for( var tab in form2.tabs ){
			...
			for( var fieldSet in tab.fieldSets ){
				...
				for( var attrib in fieldset ) {
					if ( IsSimpleValue( fieldset[ attrib ] ) && Len( Trim( fieldset[ attrib ] ) ) && ( !StructKeyExists( matchingFieldset, attrib ) || !ArrayContainsNoCase( autoFieldsetAttribs, attrib ) ) ) {
			...
			for( var attrib in tab ) {
				if ( IsSimpleValue( tab[ attrib ] ) && Len( Trim( tab[ attrib ] ) ) && ( !StructKeyExists( matchingTab, attrib ) || !ArrayContainsNoCase(autoTabAttribs, attrib ) )  ) {
